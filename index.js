        ...
        const requestFn = () => new Promise((innerResolve, innerReject) => {
            request.get({
                ...
                membersResponse.forEach((member) => {
                    ...
                    if (daysActive < daysSinceLastActive && deactivated === "False") {
                        eligible = "Yes";
                    } else {
                        eligible = "No";
                    }
...
async function beginGivingSeats() {
  ...
  const post_csvHeaders = [['Member Email', 'Member ID', 'Member Full Name', 'Days Since Last Active', 'Last Active', 'Eligible For Enterprise Seat', 'Given Enterprise Seat', 'AA ID']];
  let pre_rows; 
  try {
      ...
      return new Promise(async (resolve, reject) => {
          ...
          const requestFn = () => new Promise((resolve, reject) => {
              if (isEligible === "Yes" && isDeactivated === "False") {
                  ...
                  const giveEnterpriseSeatURL = `https://api.atlassian.com/admin/v1/orgs/${orgId}/directory/groups/${trelloGroupId}/memberships`
                  const bodyData = `{
                    "account_id": "${aaID}"
                  }`;
                  ...
                  request.post({
                      ...
                      if (response.statusCode === 200) {
                          const rowData = [email, memberId, fullName, daysActive, lastAccessed, isEligible, 'Yes', aaID];
                  ...
                  const rowData = [email, memberId, fullName, daysActive, lastAccessed, isEligible, 'No', aaID];
                  fs.appendFileSync(`post_run_member_report_${post_timestamp}.csv`, rowData.join(', ') + '\r\n');
                  resolve();
              }
          });
